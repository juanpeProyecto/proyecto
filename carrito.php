...
<body class="bg-gradient-to-br from-[#E0FAF4] to-[#51B2E0] min-h-screen w-full overflow-x-hidden px-1 sm:px-0">
    ...
    <main>
        <div class="w-full max-w-xs sm:max-w-lg md:max-w-2xl xl:max-w-4xl mx-auto mt-0 p-2 sm:p-4 md:p-6 rounded-2xl shadow-lg bg-white">
            <h1 class="text-3xl font-bold text-[#256353] text-center mb-8">Tu carrito</h1>
            <div id="contenidoCarrito" class="space-y-4"></div> 
            <div id="totalCarrito" class="text-right text-xl font-bold text-[#21476B] mt-8"></div>
            <div class="mt-8">
                <form id="formPedido" action="confirmarPedido.php" method="POST"><!--envio el formulario a confirmarPedido.php con los datos del carrito-->
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:justify-between mt-8">
                        <button id="btnVaciar" type="button" class="flex items-center justify-center gap-2 bg-[#51B2E0] hover:bg-[#72E8AC] text-white hover:text-[#256353] px-6 py-2 rounded-lg font-semibold transition-all duration-200 w-full sm:w-auto cursor-pointer"><span class="material-symbols-outlined hidden sm:inline">delete</span>Vaciar carrito</button>
                        <button id="btnFinalizar" type="submit" class="flex items-center justify-center bg-[#51B2E0] hover:bg-[#72E8AC] text-white hover:text-[#256353] px-6 py-2 rounded-lg font-semibold w-full sm:w-auto cursor-pointer">Finalizar pedido</button>
                    </div>
                </form>
            </div>
        </div>
